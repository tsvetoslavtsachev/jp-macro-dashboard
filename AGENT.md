...
двете HTML страници се раждат ЗАЕДНО. При `--export-context`: историята се
смята in-memory и НЕ се записва, журналът се ЧЕТЕ и НЕ се дописва.
...
дословна) · `test_docs` (този документ и README не дрейфват от кода).
